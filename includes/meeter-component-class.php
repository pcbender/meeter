...
class BuddyMeet_Component extends BP_Component {
	...
	public function includes( $includes = array() ) {
		...
		if ( bp_is_active( 'groups' ) ) {
			$includes[] = 'meeter-group-class.php';
		}
	 ...
	 * @uses meeter_get_slug() to get Meeter root slug
	 */
	public function setup_globals( $args = array() ) {
		...
		$globals = array(
			...
			'root_slug' => isset( $bp->pages->{$this->id}->slug ) ? $bp->pages->{$this->id}->slug : meeter_get_slug(),
    ...
    public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
        ...
        if ( bp_is_groups_component() && bp_is_single_item() ) {
            ...
            $slug = meeter_get_slug();
            $meeter_link = trailingslashit($group_link . '/' . $slug);

            $sub_nav[] = array(
                'name' => _x('Meet the Group', 'Meeter group call screen sub nav', 'meeter'),
                'slug' => 'group',
                'parent_url' => $meeter_link,
                'parent_slug' => $slug,
                'screen_function' => 'meeter_screen_group',
                'position' => 20,
                'item_css_id' => 'meeter-screen-group'
            );

            $sub_nav[] = array(
                'name' => _x('Meet Members', 'Meeter members call screen sub nav', 'meeter'),
                'slug' => 'members',
                'parent_url' => $meeter_link,
                'parent_slug' => $slug,
                'screen_function' => 'meeter_screen_members',
                'position' => 10,
                'item_css_id' => 'meeter-screen-members'
            );
    ...
    public function members_autocomplete() {
        check_ajax_referer( 'meeter_members_autocomplete' );
        ...
        $room_members = groups_get_groupmeta($group_id, Meeter::ROOM_MEMBERS_PREFIX . $room);
        if($room_members){
    ...
    public function members_add_to_invite_list() {
        check_ajax_referer( 'meeter_members_add_invite' );
        ...
        echo sprintf(
            ...
            esc_html__('Remove Invite', 'meeter')
        );
    ...
    public function members_send_invites() {
        check_ajax_referer( 'meeter_send_invites' );
            ...
            foreach ($users as $user_id) {
                if(bp_is_active( 'notifications' )) {
                    ...
                    $notification_id = bp_notifications_add_notification(array(
                        ...
                        'component_name' => meeter_get_slug(),
                ...
                $group_link = bp_get_group_permalink( $group );
                $meet_link = $group_link . meeter_get_slug() . '/members/' . $room;
                ...
                bp_send_email( 'meeter_send_invitation', (int) $user_id, $args );
            }
        }
    ...
    public function members_delete_room() {
        check_ajax_referer( 'meeter_members_delete_room' );
        ...
        $meet_link = $group_link  . 'meeter/members/';
        ...
        switch ( $action ) {
            case 'members_send_invites':
                ...
                if($requested_user_id === $current_user_id){
                    $text = sprintf( __( '%s: You sent a meet request', 'meeter' ), $group->name );
                } else {
                    $text = sprintf( __( '%s: User %s sent you a meet request', 'meeter' ), $group->name, $user_fullname );
                }

                $group_link = bp_get_group_permalink( $group );

                $notification_link = $group_link . meeter_get_slug() . '/members/' . $room;

                bp_notifications_mark_notification( $notification_id, false );

                return apply_filters( 'meeter_' . $action . '_notification', '<a href="' . $notification_link . '">' . $text . '</a>', $group_link, $user_fullname, $group->name, $text, $notification_link );
                break;
            default:
                $custom_action_notification = apply_filters( 'meeter_' . $action . '_notification', null, $item_id, $secondary_item_id, $total_items, $format );
    ...
    public function add_users_to_room($group_id, $users, $room_id = null, $room_name = null){
        ...
        $room_users_option_key = Meeter::ROOM_MEMBERS_PREFIX . $room['id'];
    ...
    public function remove_users_from_room($group_id, $users, $room_id){
        ...
        foreach($users as $user_id){
            ...
            $room_members_option_key = Meeter::ROOM_MEMBERS_PREFIX . $room_id;
 ...
 * @uses buddypress()
 */
function meeter_load_component() {
	buddypress()->meeter = new BuddyMeet_Component;
}
add_action( 'bp_loaded', 'meeter_load_component' );
