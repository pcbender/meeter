...
Meeter is a BuddyPress (2.5+) plugin that uses [Jitsi Meet](https://jitsi.org/jitsi-meet/) to allow the members of a community to participate into virtual conference rooms with video and audio capabilities. Meeter's features include:
...
Moreover, you can use the shortcode [meeter room=ROOM_HERE subject=SUBJECT_HERE] to add a conference room to any wordpress page. In that case, you have to pass any configuration by using the following shortcode parameters:

* domain: The domain of the Jitsi Meet installation. Meeter uses by default the meet.jit.si free service.
...
Meeter uses by default the meet.jit.si service which is maintained by the Jitsi team at 8x8. Upon the initialization of a room, Meeter sends the following information to the service:
...
You can download and install Meeter using the built in WordPress plugin installer. If you download Meeter manually, make sure it is uploaded to "/wp-content/plugins/meeter/".
...
Meeter is mainly a BuddyPress plugins. It actually extends the BuddyPress Groups component by adding a new Meeter menu page as well as a settings page accessible via the Manage menu of the Group. However, if you want to use Meeter in any other WordPress page you can use the [meeter] shortcode. In that case the plugin just adds a room in the respective page by using the passed configuration parameters.
...
* Fixed the issue of disabling buttons that co-exist in the same page with the meeter short code
...
* Fixed an issue causing the [meeter] short code not to work properly
* Added documentation about the configuration parameters of the [meeter] short code
* Updated the FAQ
