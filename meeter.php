...
if ( ! class_exists( 'Meeter' ) ) :
/**
 * Main Meeter Class
 */
class Meeter {

    const USER_ROOMS_PREFIX = 'meeter_user_room_';
    const ROOM_MEMBERS_PREFIX = 'meeter_room_members_';
	 ...
	 * @since 1.0.0
	 *
	 * @uses Meeter::setup_globals() to set the global needed
	 * @uses Meeter::includes() to include the required files
	 * @uses Meeter::setup_actions() to set up the hooks
	 * @return object the instance
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			self::$instance = new Meeter;
	 ...
	 * @uses plugin_dir_path() to build Meeter plugin path
	 * @uses plugin_dir_url() to build Meeter plugin url
	 */
	private function setup_globals() {
		...
		$this->file       = __FILE__;
		$this->basename   = apply_filters( 'meeter_plugin_basename', plugin_basename( $this->file ) );
		$this->plugin_dir = apply_filters( 'meeter_plugin_dir_path', plugin_dir_path( $this->file ) );
		$this->plugin_url = apply_filters( 'meeter_plugin_dir_url',  plugin_dir_url ( $this->file ) );

		// Includes
		$this->includes_dir = apply_filters( 'meeter_includes_dir', trailingslashit( $this->plugin_dir . 'includes'  ) );
		$this->includes_url = apply_filters( 'meeter_includes_url', trailingslashit( $this->plugin_url . 'includes'  ) );

		// Languages
		$this->lang_dir  = apply_filters( 'meeter_lang_dir', trailingslashit( $this->plugin_dir . 'languages' ) );

		// Meeter slug and name
		$this->meeter_slug = apply_filters( 'meeter_slug', 'meeter' );
		$this->meeter_name = apply_filters( 'meeter_name', 'Meeter' );
	...
	private function includes() {
		require( $this->includes_dir . 'meeter-actions.php'         );
		require( $this->includes_dir . 'meeter-functions.php'       );

		//TODO CHECK ADMIN INTERFACES
		/*if( is_admin() ){
			require( $this->includes_dir . 'admin/meeter-admin.php' );
		}*/
	}
	...
	private function setup_actions() {
		...
		add_action( 'activate_'   . $this->basename, 'meeter_activation'   );
		add_action( 'deactivate_' . $this->basename, 'meeter_deactivation' );
        ...
        add_shortcode( 'meeter', array($this, 'add_shortcode'));

		do_action_ref_array( 'meeter_after_setup_actions', array( &$this ) );
	}
    ...
    public function set_default_groups_nav() {
        bp_core_new_nav_default (
            array(
                'parent_slug'       => meeter(),
                'subnav_slug'       => 'members',
                'screen_function'   => 'meeter_screen_members'
            )
        );
    }
	...
	public function load_textdomain() {
		$locale = apply_filters( 'meeter_load_textdomain_get_locale', get_locale(), $this->domain );
		$mofile = sprintf( '%1$s-%2$s.mo', $this->domain, $locale );
		$mofile_global = WP_LANG_DIR . '/meeter/' . $mofile;
    ...
    public function enqueue_styles(){
        if(function_exists( 'buddypress' )) {
            global $bp;
            if (meeter_get_slug() === $bp->current_action) {
                $sub_action = meeter_get_current_action();
                if ('members' === $sub_action) {
                    //Enqueue the jquery autocomplete library
                    wp_enqueue_style('meeter-invites-css', meeter_get_plugin_url() . "assets/css/invites.css", '', meeter_get_version(), 'screen');
                }
            }
        }
    }
    ...
    public function enqueue_scripts(){
        ...
        if(is_page()){
            $post = get_post();
            if($post && has_shortcode($post->post_content, meeter_get_slug())){
                $load_scripts = true;
        ...
        if($load_scripts){
            ...
            wp_enqueue_script( 'meeter-invites-js', meeter_get_plugin_url()  . 'assets/js/invites.js', array( 'jquery-ui-autocomplete' ) );
            wp_localize_script('meeter-invites-js', 'args', array(
                'ajaxurl' =>  admin_url( 'admin-ajax.php', 'relative' )
            ));

            $handle = 'meeter-jitsi-js';
            wp_enqueue_script( $handle, "https://meet.jit.si/external_api.js", array(), meeter_get_version(), true);
        }
    }
	...
	public function load_component() {
		if ( self::bail() ) {
			...
			require( $this->includes_dir . 'meeter-component-class.php' );
		}
	}
	...
	public static function config_check() {
		...
		$meeter = plugin_basename( __FILE__ );
		...
		self::$bp_config['network_active'] = isset( $network_plugins[ $meeter ] );
	...
	public function warning() {
		...
		if ( ! self::version_check() ) {
			$warnings[] = sprintf( esc_html__( 'Meeter requires at least version %s of BuddyPress.', 'meeter' ), self::$required_bp_version );
			$resolve[]  = sprintf( esc_html__( 'Upgrade BuddyPress to at least version %s', 'meeter' ), self::$required_bp_version );
		}
		...
		if ( ! $config['blog_status'] ) {
			$warnings[] = esc_html__( 'Meeter requires to be activated on the blog where BuddyPress is activated.', 'meeter' );
			$resolve[]  = esc_html__( 'Activate Meeter on the same blog than BuddyPress', 'meeter' );
		}

		if ( ! $config['network_status'] ) {
			$warnings[] = esc_html__( 'Meeter and BuddyPress need to share the same network configuration.', 'meeter' );
			$resolve[]  = esc_html__( 'Make sure Meeter is activated at the same level than BuddyPress on the network', 'meeter' );
		}
		...
		if ( ! empty( $warnings ) ) {
			...
			if ( is_super_admin() ) {
				...
				$deactivate_link = '<a href="' . esc_url( $deactivate_link ) . '">' . esc_html__( 'deactivate', 'meeter' ) . '</a>';
				$resolve_message = '<ol><li>' . sprintf( __( 'You should %s Meeter', 'meeter' ), $deactivate_link ) . '</li>';
				...
				if ( $config['network_status'] && $config['blog_status']  )
					$resolve_message .= '<li>' . esc_html__( 'Once done try to activate Meeter again.', 'meeter' ) . '</li></ol>';
    ...
    /**
     * Registers the meeter shortcode
     * @param $params
     */
    public function add_shortcode($params) {
        $params = apply_filters('meeter_custom_settings', $params);
        $params = wp_parse_args($params, meeter_default_settings());
        $script = sprintf(
...
function meeter() {
	return meeter::instance();
}

meeter();

/**
 * Meeter unistall Hook registration
 */
register_uninstall_hook( __FILE__, 'meeter_uninstall' );
